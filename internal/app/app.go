...
func Run() {
	...
	analyticsCache := cache.NewAnalyticsCache(redisClient, 5*time.Minute)
	txService := service.NewTransactionService(txRepo, analyticsCache)
	analyticsService := service.NewAnalyticsService(txRepo, analyticsCache)
